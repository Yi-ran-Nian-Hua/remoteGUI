...
def send_request(tab_name, tab_data):
    params = []
    if not tab_data["param_container"]:
        print("参数容器未初始化")
        return
...
def create_new_tab(tabs, tab_bar, tab_area):
    ...
    tab_data["add_param_row"] = lambda: add_param_row(tab_data)
...
with ui.row().classes('w-full h-screen'):
    ...
    with ui.column():
        ...
        with ui.row():
            ...
            ui.button('搜索')
        # 下方展示数据区域
        # TODO: 根据change_status的状态进行更新
        with ui.scroll_area().classes("h-screen"):
            display_area = ui.label('下方展示数据区域' * 100000)
